...
void pathcandupd_cb(const nav_msgs::Path::ConstPtr& msg){
  ...
  if(msg->poses.size() > 0){
    for(int i = 0; i < msg->poses.size(); i++){
      ...
      cv::Scalar candidate_color_alt;
      candidate_color_alt = candidate_color;
      candidate_color_alt[2] = msg->poses[i].pose.position.z * 5;
      draw_pose2d(msg->poses[i].pose.position,2,candidate_color_alt,false,tf::getYaw(msg->poses[i].pose.orientation));
    }
  }
}
