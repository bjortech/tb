...
void draw_path(nav_msgs::Path pathin,cv::Scalar color,bool copy,bool draw_poses){
  if(pathin.poses.size() > 2){
  	for(int i = 0; i < pathin.poses.size();i++){
  		if(abs(pathin.poses[i].pose.position.z - pos.z) < 10){
        draw_pose2d(pathin.poses[i].pose.position,2,color,true,tf::getYaw(pathin.poses[i].pose.orientation));
...
void draw_grids(std::vector<int> grids_to_draw,cv::Scalar color){
	if(grids_to_draw.size() < 2)
		return;
	for(int i = 0; i < grids_to_draw.size()-1; i++){
    draw_rectangle32(polygon_grids[grids_to_draw[i]].polygon.points[1],polygon_grids[grids_to_draw[i]].polygon.points[3],color,true);
    draw_rectangle32(polygon_grids[grids_to_draw[i]].polygon.points[1],polygon_grids[grids_to_draw[i]].polygon.points[3],color,false);
  }
...
void create_polygon_grids(){
  ...
  for(int y = 0; y < num_gridsprside; y++){
	...
	draw_grids(grids_to_draw,grid_color);
}
...
//  geometry_msgs::PolygonStamped polycircle = polyCircle(pos,par_eval_maxrange,8);
  ...
  //  ROS_INFO("Scan cb %i possible_grids %i hits_possible_grids %i %i",scan->ranges.size(),possible_grids.size(),hits_possible_grids.size(),polycircle.polygon.points.size());
    ...
    //      int points = fmin(hitcount*5,250);
      //    if(current_number > 0 && current_number < polygon_grids.size()){
          //  draw_rectangle32(polygon_grids[current_number].polygon.points[1],polygon_grids[current_number].polygon.points[3],cv::Scalar(points,points,points),false);
            hitcount = 0;
  ...
  std::vector<int> grids_to_draw;
    ...
    if(grids_to_draw.size() > 1)
     ...
     draw_polygon(scan2poly(*scan,pos), path_color,false);
	}
       draw_polygon(scan2poly(*scan,pos), path_color,true);
}
void create_polygon_grids(){
	std::vector<int> grids_to_draw;
	geometry_msgs::Point p;
  grids_to_draw.resize(0);
...
void get_zdown_grids(){
  octomap::point3d closestObst;
	int gn = 0;
  for(int i = 0; i < grids_centroids.size(); i++){
		geometry_msgs::Point xy;
		xy = grids_centroids[gn];
		if(xy.x > xmin && xy.y > ymin &&
			 xy.x < xmax && xy.y < ymax){
	      int z = zmax;
	      float dst = 1.1;
	      while(dst == 5 && z > zmin){
	        ...
	        z--;
				...
				std::vector<int> g;
				g.push_back(gn);
				draw_grids(g,cv::Scalar(0,0,100+colout));
				colorize_grid_dst(gn,colout*2);
				}
			gn++;
		}
    //
...
void draw_grids_at_ranges(geometry_msgs::Point  midpoint){
  ...
  for(int i = 0; i < grids_at_ranges.size(); i++){
    for(int k = 0; k < grids_at_ranges[i].size(); k++){
      draw_grids(grids_at_ranges[i],cv::Scalar(20,20,100));
    }
  }
}

void draw_path(nav_msgs::Path pathin, cv::Scalar color,bool copy){
  for(int k = 0; k < pathin.poses.size(); k++){
      draw_pose2d(pathin[i].pose.position,2,color,copy,tf::getYaw(pathin.poses[i].pose.orientation));
  }
}

void draw_path_vstdcnd(int zn0,int zn1,bool copy){
  cv::Color color_visited;
  cv::Color color_candidates;
  color_visited = visited_color;
  color_candidates = candidate_color;
  for(int i = z0; i < z1; i++){
    if(paths_cand_at_lvl[i].poses.size() > 1){
      draw_path(paths_cand_at_lvl[i],candidate_color,copy);
    }
    if(paths_vstd_at_lvl[i].poses.size() > 1){
      draw_path(paths_vstd_at_lvl[i],visited_color,copy);
    }
  }
}
...
void cmd_cb(const geometry_msgs::PoseStamped::ConstPtr& msg){
  ...
  target_color_alt[0] = msg->pose.position.z * 5;
  draw_line(target.pose.position,msg->pose.position,target_color,false);
  last_target = target;
  target      = *msg;
...
bool first = true;
  while(ros::ok()){
	...
	//	if((ros::Time::now() - last_check).toSec() > 2 && got_map && done){
		...
		//transform_visited();
    //draw_pose2d(pos,2,visited_color,true,pos_yaw);
    draw_path_vstdcnd(2,10,copy);

  //draw_path(path_visited_tar01,visited_color,true,true);
  ...
  get_zdown_grids();
    ...
    if(first){
      ...
      cv::imwrite(par_workdir+"/global_img.png",img_copy);
