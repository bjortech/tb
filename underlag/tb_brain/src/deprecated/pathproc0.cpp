...
std::vector<int> getinpath_indexes_inrad(nav_msgs::Path pathin,std::vector<bool> indexes_checked,float radians){
  ...
  for(int i = 0; i < pathin.poses.size(); i++){
    for(int k = 0; k < indexes_checked.size(); k++){
      if(!indexes_checked[k] && k != i){
        float dist = get_dst2d(pathin.poses[i].pose.position,pathin.poses[k].pose.position);
        if(dist < radians)
          vec_out.push_back(i);
      }
    }
...
std::vector<std::vector<int>> get_segmented_clusters_in_path2d(nav_msgs::Path pathin,float radians){
  ...
  for(int i = 0; i < pathin.poses.size(); i++){
    if(!indexes_accounted_for[i]){
      ...
      indexes_in_cluster = getinpath_indexes_inrad(pathin,indexes_accounted_for,radians);
      ROS_INFO("indexes_in_cluster: %i",indexes_in_cluster.size());
      for(int k = 0; k < indexes_in_cluster.size(); k++){
        indexes_accounted_for[indexes_in_cluster[k]] = true;
      }
