...
int zlvl,xmin,ymin,zmin,xmax,ymax,zmax,range_x,range_y,range_z;
bool got_map;
...
std::vector<nav_msgs::Path> paths_at_zn,paths_clustered_at_zn;
...
nav_msgs::Path get_path_segmented2d(nav_msgs::Path pathin,float radians){
  ...
  ROS_INFO("PathProc get_path_segmented2d: %i",pathout.poses.size());
  return pathout;
}
nav_msgs::Path unify_path(){
  ...
  for(int i = 0; i < paths_clustered_at_zn.size()-1; i++){
		ROS_INFO("paths_clustered_at_zn[%i]: size: %i",i,paths_clustered_at_zn[i].poses.size());
...
//  ROS_INFO("Time_taker: Now timed: %s. It took %.5f seconds",activity.c_str(),dt);
  if(activity == "done"){
    float total_dt;
    for(int i = 0; i < tasks_durations.size();i++){
      total_dt += tasks_durations[i];
    }
    ROS_INFO("Time_taker: Total time: %.5f",total_dt);
    for(int i = 0; i < tasks_durations.size();i++){
      int percent = round((tasks_durations[i] / total_dt)*100)
      ROS_INFO("Time_taker: %s: %i percent (%.3f seconds)",tasks[i].c_str(),percent,tasks_durations[i]);
    }
    tasks.resize(0);
...
void update_path()
{
  ...
  for(int zn = min_zlvl; zn < max_zlvl; zn++){
    int z = z_lvls[zn];
    //start_with("going through zn at: " + std::to_string(zn) + " lvl");
    update_edto(pos,par_maxobs+1,par_maprad,z-0.5,z+0.5,false);
        ...
        for(int x = xmin; x < xmax; x++){
          ...
          if(dst_point_in_path_lim(path_candidates,ps.pose.position,5)){
            ...
            if(round((par_minobs + par_maxobs)/2) == round(d)){
              ...
              paths_at_zn[zn].poses.push_back(ps);
      ...
      //start_with("clustering path at: " + std::to_string(zn) + " lvl");
      paths_clustered_at_zn[zn] = get_path_segmented2d(paths_at_zn[zn],radians);
    }
    //start_with("Unifying clustered path");
    paths_clustered_and_unified = unify_path();
    pub.publish(paths_clustered_and_unified);
  //start_with("done");
...
void visited_cb(const nav_msgs::Path::ConstPtr& msg){
  path_visited = *msg;
  pos = path_visited.poses[-1].pose.position;
}
...
int main(int argc, char **argv)
{
  ...
  for(int i = 0; i < 20; i++){
    ...
    paths_at_zn.push_back(path_template);
  ...
  ros::Publisher pub_orig      = nh.advertise<nav_msgs::Path>("/tb_path_filtered/full",100);
  ros::Publisher pub_path_un   = nh.advertise<nav_msgs::Path>("/tb_path_filtered/unified",100);
  ...
  path_in_lvls.header = hdr();
  path_in_bbpoly.header = hdr();
  path_not_visited.header = hdr();
  path_candidates_notvstd.header = hdr();
  bool use3dnot2d;
  float cutoff_dst = 10;
  while(ros::ok()){
    if(got_map){
      ...
      path_candidates_notvstd = constrain_path_vstd(path_candidates,path_visited,cutoff_dst/2,use3dnot2d);
      ...
      if(path_visited.poses.size() > 10)
        path_not_visited = constrain_path_vstd(path_candidates_notvstd,path_visited,cutoff_dst,use3dnot2d);
      else
      ...
      pub_path_un.publish(paths_clustered_and_unified);
    }
    rate.sleep();
	  ros::spinOnce();
  }
