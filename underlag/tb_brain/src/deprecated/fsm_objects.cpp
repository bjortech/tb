...
void create_floor(){
  ...
  building_n_active,building_active,at_floor,dst_target,building_v0, visited_path.poses.size(),altlvl_msg.data,alt_target.data);
  ...
  for(int i = v0; i < vn; vn++){
    ...
    || get_dst3d(visited_path.poses[i].pose.position,visited_path.poses[last_i].pose.position) > 20){
      ...
      buildings_floorpaths_shorter[building_n_active][zn].poses.push_back(visited_path.poses[i]);
      ROS_INFO("New Floor Polygon[%i]: [%i points of %i]",i,poly_building.polygon.points.size(),buildings_floorpaths[building_n_active][zn].poses.size());
    }
  }

  for(int i = building_n0_targets; i < targetcmds_sent.size(); i++){
  ...
  int numtarget = targetcmds_sent.size() -  building_n0_targets;
  if((numtarget - path_floor_below.poses.size() ) < 2)
    building_floors_known.push_back(zn-1);
  if((numtarget - path_floor_above.poses.size() ) < 2)
    building_floors_known.push_back(zn+1);
  else
  ...
  buildings_floorpaths[building_n_active][zn].poses.size(),buildings_floorpaths[building_n_active][zn-1].poses.size(),
  buildings_floorpaths[building_n_active][zn+1].poses.size());
  ...
  i_type_num_v0_vn_z0_zn.push_back(std::make_tuple(int(i_type_num_v0_vn_z0_zn.size()),type_building,building_n_active,building_v0,int(visited_path.poses.size()),zn,zn));
...
void update_centroid(){
  ...
  centroid_sum_count = visited_path.poses.size() - building_v0;
  ...
  int count = 0;
  int centroid_sum_count = visited_path.poses.size() - building_v0;
  float a_pr_i = rad2deg * (2*M_PI / radians_segmentsize);

  if(centroid_sum_count > 10){
    for(int i = building_v0; i < visited_path.poses.size(); i++){
...
int main(int argc, char** argv)
{
  ...
  while(ros::ok()){
    ...
    /*    path_filtered.header.frame_id = "map";
        ...
        building_n_active,building_active,at_floor,dst_target,building_v0, visited_path.poses.size(),altlvl_msg.data,alt_target.data,d);

        if(!building_active && !at_floor){
          if(targetcmds_sent.size() > 0 && dst_target < 5 && d < 25 && d > 0){
            activate_building();
          }
        }
        else if(at_floor){
