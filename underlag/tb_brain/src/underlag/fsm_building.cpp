...
int building_state,last_update_vlen,mainstate,missionstate,zlvl,building_active_floor,building_n_active,building_floor_v0,building_floor_n0_targets,building_v0,building_n0_targets,last_completion;
ros::Publisher pub_completion,pub_floorchange,pub_poly,pub_centroid,pub_bldstate,pub_path_floor;
...
nav_msgs::Path path_floor,building_path,path_visited,path_filtered,path_candidates,path_floor_above,path_floor_below;
...
std::vector<int> getinpath_indexes_inrad_orig(nav_msgs::Path pathin,geometry_msgs::Point pos_to_check,float radians){
...
void create_floor(){
  ...
  for(int i = building_floor_n0_targets; i < targetcmds_sent.size(); i++){
    ...
    if(p.z > 0 && p.x + p.y != 0){
      ...
      std::vector<int> indexes_in_rad = getinpath_indexes_inrad_orig(path_candidates,targetcmds_sent[i].pose.position,5.5);
...
void buildingstate_cb(const std_msgs::UInt8::ConstPtr& msg){
  building_state = msg->data;
  if(msg->data != building_state){
    building_state = 3;
    if(msg->data == 3){
      create_floor();
    }
...
void vstdpath_cb(const nav_msgs::Path::ConstPtr& msg){
  path_visited = *msg;
  int vlen = path_visited.poses.size();
  if(path_visited.poses.size() > 3)
    last_pose = path_visited.poses[vlen - 1];
  if(building_active){
    int vlen_floor = vlen - building_floor_v0;
    int vlen_floor_delta = vlen_floor - path_floor.poses.size();
    if(vlen_floor_delta > 0){
      for(int i = 0; i < vlen_floor_delta; i++){
        int i_to_push =  vlen - vlen_floor_delta + 1;
        if(i_to_push < msg->poses.size() && i_to_push > 0)
          path_floor.poses.push_back(msg->poses[i_to_push]);
      }
    }
    ROS_INFO("AFloorpath ppended %i poses",vlen_floor_delta);
    if(vlen > last_update_vlen + 3){
      int completion_init = get_completion_percent(building_floor_v0,path_visited.poses.size(),building_centroid.point);
      int completion_new  = get_completion_percent(building_floor_v0,path_visited.poses.size(),building_centroid_initial.point);
      std_msgs::UInt8 completion_msg;
      completion_msg.data = fmin(completion_init,completion_new);
      int dc = completion_msg.data - last_completion;
      if(dc > 10){
        last_completion = completion_msg.data;
        building_centroid = get_centroid(building_floor_v0,path_visited.poses.size());
        pub_centroid.publish(building_centroid);
      }
      else
        pub_completion.publish(completion_msg);
    }
    else{
      pub_path_floor.publish(path_floor);
    }
...
void bpathbbpoly_cb(const nav_msgs::Path::ConstPtr& msg){
  path_candidates = *msg;
  create_floor();
}
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber s7  = nh.subscribe("/tb_fsm/building_state",100,&buildingstate_cb);
  ros::Subscriber s10 = nh.subscribe("/tb_bld/centroid_initial",100,&centroidinitial_cb);
  ros::Subscriber s8  = nh.subscribe("/tb_path/bbpoly",100,&bpathbbpoly_cb);
  ...
  pub_completion  = nh.advertise<std_msgs::UInt8>("/tb_bld/completion_percent",100);
