...
nav_msgs::Path path_2_path_lvls(nav_msgs::Path pathin){
  ...
  for(int i = 0; i < pathin.poses.size(); i++){
    ...
    for(int  k = 0; k < indexes_in_rad.size(); k++){
      if(z_lvls[int(znmin_znmax_num.x)] < pathin.poses[indexes_in_rad[k]].pose.position.z
      && z_lvls[int(znmin_znmax_num.y)] > pathin.poses[indexes_in_rad[k]].pose.position.z)
        indexes_num++;
    }
  }
  if(indexes_num > znmin_znmax_num.z){
...
if(constrain_minlvls_active){
  pathin = path_2_path_lvls(pathin);
}
if(constrain_bbpoly_active){
  pathin = path_2_path_bb(pathin,poly_bb);
}
...
geometry_msgs::Vector3 znmin_znmax_num;
bool constrain_minlvls_active,constrain_bbpoly_active;
void constrain_minlvls_cb(const geometry_msgs::Vector3::ConstPtr& msg){
  znmin_znmax_num = *msg;
}

ROS_INFO("Constrain active: Will check for multiple lvls ranging from %.0f -> %.0f with %.0f as min num",msg->x,msg->y,msg->z);
ROS_INFO("Constrain active: Will check for bbpoly ");


void constrain_bbpoly_cb(const geometry_msgs::PolygonStamped::ConstPtr& msg){
  path_bb_active = true;
  poly_bb = *msg;
}

void active_constrain_minlvls_cb(const std_msgs::Bool::ConstPtr& msg){
  constrain_minlvls_active = msg->data;
}

void active_constrain_bbpoly_cb(const std_msgs::Bool::ConstPtr& msg){
  constrain_bbpoly_active = msg->data;
}
...
nav_msgs::Path getcandidates_withinconstrains(){
  nav_msgs::Path pathout;
	pathout.header.frame_id = "map";
  if(pathin.poses.size() == 0){
...
void checktf(){
  ...
  if(sqrt(pow(transformStamped.transform.translation.x - last_pose.pose.position.x,2)+ pow(transformStamped.transform.translation.y - last_pose.pose.position.y,2)+
    ...
    int zlvl  = last_pose.pose.position.z / par_zjump + 3;
...
void add_path_candidates(nav_msgs::Path pathin){
  for(int i = 0; i < pathin.poses.size(); i++){
    if(dst_point_in_path_lim(path_candidates,pathin.poses[i].pose.position,5)){
      pathcand.poses.push_back(pathin.poses[i]);
...
void custorg_cb(const nav_msgs::Path::ConstPtr& msg){
  last_custpath = ros::Time::now();
  nav_msgs::Path pathout;
  float radius = 6;
  if(msg->poses.size() > 2){
    std::vector<std::vector<int>> clusters;
    clusters = get_segmented_clusters_in_path2d(*msg,radius);
    ROS_INFO("clusters: %i",clusters.size());
    ...
    pathout = getinpath_not_visited2d(paths_cand_at_lvl[zlvl],paths_vstd_at_lvl[zlvl],10)
    pathout.header.frame_id ="map";
    custpath_pub.publish(pathout);
  }
}

nav_msgs::Path gather_candidates(){
  nav_msgs::Path pathout;
  pathout.header.frame_id ="map";
  for(int i = 0; i < paths_cand_at_lvl.size()-1; i++){
    for(int k = 0; k < paths_cand_at_lvl[i].poses.size(); k++){
      if(znmin_znmax_num.x < i && znmin_znmax_num.y > i){
        geometry_msgs::Point p;
        p  = paths_cand_at_lvl[i].poses[k].pose.position;
        yw = tf::getYaw(paths_cand_at_lvl[i].poses[k].pose.orientation);
        if(in_poly(poly_bb,p.x,p.y)){
          if(constrain_minlvls_active
        }
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber s52 = nh.subscribe("/tb_fsm/path_to_process",1,&custorg_cb);
//  ros::Subscriber s4 = nh.subscribe("/tb_path/switch_candidate_path_onoff",1,&candswitch_cb);
//  ros::Subscriber s51 = nh.subscribe("/tb_nav/path_visited",1,&pathvstd_cb);
  ...
  while(ros::ok()){
    checktf();
    if(done && (ros::Time::now() - last_custpath).toSec() > 3 && paths_cand_at_lvl[zlvl].poses.size() > 0){
      done = false;
