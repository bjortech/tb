...
ros::Publisher pub_path_closest_dst,pub_path_closest_obstacle,pub_point_closest_dst,pub_point_closest_obstacle;
...
void get_path_closest(nav_msgs::Path pathin,float collision_radius,float min_dst, bool get_point){
  ...
  if(update_edto_bbvec(bbvec,collision_radius)){
	...
	if(get_point)
		pub_path_closest_obstacle.publish(closest_obst);
	else
		pub_path_closest_dst.publish(closest_dst);
}

void get_point_closest(geometry_msgs::Point pnt,float collision_radius,float min_dst, bool get_point){
	geometry_msgs::PointStamped closest_obst;
	std_msgs::Float64 closest_dst;
	closest_obst.header.frame_id = "no_obstacle";
	closest_obst.header.stamp 	 = ros::Time::now();
  ...
  if(update_edto_bbvec(bbvec,collision_radius)){
	...
	if(get_point)
		pub_point_closest_obstacle.publish(closest_obst);
	else
		pub_point_closest_dst.publish(closest_dst);
}
void get_path_closest_cb(const nav_msgs::Path::ConstPtr& msg){
	if(got_map){
		get_path_closest(*msg,10,5,false);
	}
}
void get_point_closest_cb(const geometry_msgs::PointStamped::ConstPtr& msg){
	if(got_map){
		get_point_closest(msg->point,20,10,false);
	}
}
void get_path_closest_obstacle_cb(const nav_msgs::Path::ConstPtr& msg){
	if(got_map){
		get_path_closest(*msg,20,10,true);
	}
}
void get_point_closest_obstacle_cb(const geometry_msgs::PointStamped::ConstPtr& msg){
	if(got_map){
		get_point_closest(msg->point,20,10,true);
	}
}
int main(int argc, char** argv)
{
  ros::init(argc, argv, "tb_edto_obstacledistance_node");
  ros::NodeHandle nh;
	ros::NodeHandle private_nh("~");
	private_nh.param("sphere_radius",par_sphere_radius, 30.0);
	ros::Subscriber s1  = nh.subscribe("/octomap_full",1,octomap_callback);

	ros::Subscriber s2  = nh.subscribe("/tb_obstacles/get_path_closest_dst",1,get_path_closest_cb);
	ros::Subscriber s3  = nh.subscribe("/tb_obstacles/get_point_closest_dst",1,get_point_closest_cb);
