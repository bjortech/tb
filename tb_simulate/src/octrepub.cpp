...
void oct2_cb(const sensor_msgs::PointCloud2::ConstPtr& msg){
  if(!got_map){
    pc1.header = msg->header;
    pc1.points.resize(msg->data.size());
  	sensor_msgs::PointCloud2ConstIterator<float> iter_x(*msg, "x");
...
{
  ros::init(argc, argv,"tb_octorepub_node");
  ros::NodeHandle nh;
	tf2_ros::TransformListener tf2_listener(tfBuffer);

  ros::Subscriber s5  = nh.subscribe("/octomap_point_cloud_centers",  100,&oct_cb);
  //ros::Subscriber s8  = nh.subscribe("/tb_pc2_full",  100,&oct2_cb);
	ros::Subscriber s6  = nh.subscribe("/tb_path/cleared_poly",  100,&poly_cb);
	ros::Publisher octomap_repub = nh.advertise<sensor_msgs::PointCloud2> ("/octomap_repub", 1);
  ...
  ros::Rate rate(1);
	while(ros::ok()){
		...
		if(got_map){
			checktf();
      sensor_msgs::PointCloud  pc1_filt = get_pc_within();
      sensor_msgs::PointCloud2 pc2_filt = pc1_to_pc2(pc1_filt);
      pub2.publish(pc2_filt);
			octomap.header.stamp = ros::Time::now();
			octomap_repub.publish(octomap);
		}
  }
  return 0;
}
