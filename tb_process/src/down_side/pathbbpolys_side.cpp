...
{
  ros::init(argc, argv, "tb_process_bbpolys_side_node");
  ros::NodeHandle nh;
	ros::NodeHandle private_nh("~");
	private_nh.param("num_points_bbpolys",   par_num_points, 32);
  private_nh.param("extra_radius_bbpolys", par_extra_length, 5.0);//*2.0);
	ros::Subscriber os = nh.subscribe("/tb_process/get_paths_side_bbpolys",1,get_paths_bbpolys_cb);
	ros::Subscriber o1 = nh.subscribe("/tb_process/get_path_side_bbpoly",1,get_path_bbpoly_cb);
	pub_polygons = nh.advertise<tb_msgsrv::Polygons>("/tb_process/paths_side_bbpolys",10);
	pub_polygon   = nh.advertise<geometry_msgs::PolygonStamped>("/tb_process/path_side_bbpoly",10);
	ros::spin();
	return 0;
}
//
