...
{
  ros::init(argc, argv, "tb_process_clusters_side_node");
  ros::NodeHandle nh;
  ros::NodeHandle private_nh("~");
	ros::Subscriber os = nh.subscribe("/tb_process/get_path_side_clusters",1,path_to_segment_cb);
	pub_paths = nh.advertise<tb_msgsrv::Paths>("/tb_process/path_side_clusters",10);
	ros::spin();
	return 0;
}
//
