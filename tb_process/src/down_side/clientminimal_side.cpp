...
{
  ros::init(argc, argv, "tb_process_client_side_node");
  ros::NodeHandle nh;
  ros::NodeHandle private_nh("~");
	ros::Subscriber os2 = nh.subscribe("/tb_process/path_side_clusters",1,path_clusters_cb);
	ros::Subscriber os1 = nh.subscribe("/tb_process/paths_side_bbpolys",1,path_bbpolys_cb);

	pub_path_to_cluster = nh.advertise<nav_msgs::Path>("/tb_process/get_path_side_clusters",10);
	pub_paths_to_bbpoly = nh.advertise<tb_msgsrv::Paths>("/tb_process/get_paths_side_bbpolys",10);
	ros::spin();
	return 0;
}
//
