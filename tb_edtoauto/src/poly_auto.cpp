...
void raycast_circle_edto(geometry_msgs::Point midpoint,float collision_radius){
	...
	path_raycast.poses.resize(0);
	poly_cleared.polygon.points.resize(num_rays);
	...
	for(int i  = 0; i < num_rays; i++){
		...
		while(cur_ray_len < par_maprad && distance > par_min_distance){
			...
			&& cur_vec.x() < float(oct_xmax) && cur_vec.y() < float(oct_ymax)){
				...
				if(distance < collision_radius && closestObst.z() > 0 && !std::isnan(closestObst.z())){
					ps.pose.position 	 	= e_to_pnt(cur_vec);
					ps.pose.orientation = get_quat(p_to_pnt(closestObst),e_to_pnt(cur_vec));
					closest_obstacle = p_to_pnt32(closestObst);
					if(par_track_only_last){
						if(distance < par_min_distance){
							path_raycast.poses.push_back(ps);
							poly_obstacles.polygon.points.push_back(closest_obstacle);
						}
					}
					else{
						path_raycast.poses.push_back(ps);
  ...
  ros::init(argc, argv, "tb_edto_poly_auto2_node");
	...
	while(ros::ok()){
		...
		if(poly_cleared.polygon.points.size() > 0){
			pub_path.publish(path_raycast);
			pub_poly_cleared.publish(poly_cleared);
