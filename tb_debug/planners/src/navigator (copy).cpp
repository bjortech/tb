...
std::vector<int> bb_xy_xy(std::vector<int> bbmnmnx_rc){
  std::vector<int> bbmnmx_xy;
  bbmnmx_xy.resize(4);
  bbmnmx_xy[0] = c2x(bbmnmnx_rc[1]);
  bbmnmx_xy[1] = r2y(bbmnmnx_rc[2]);
  bbmnmx_xy[2] = c2x(bbmnmnx_rc[3]);
  bbmnmx_xy[3] = r2y(bbmnmnx_rc[0]);
  return bbmnmx_xy;
}
...
void update_mbmap_img(std::vector<int> bbxyxy,float zcutoff,bool use_update){
  if(use_update)
    bbxyxy = bb_xy_xy(get_bbmnmnx(false));
  ...
  for (int y = ymin; y < ymax; y++){
    for (int x = xmin; x < xmax; x++){
      if(use_update && img_update_copy.at<cv::Vec3b>(y2r(y),x2c(x))[2] > zlim)
        update.data[i++] = 100;
      else if(!use_update && img_height.at<cv::Vec3b>(y2r(y),x2c(x))[2] > zlim)
        update.data[i++] = 100;
      else
        update.data[i++] = 0;
    }
...
void test_updatembma(geometry_msgs::Point midpoint,float radlen_xy,float zcutoff, bool use_update){
  ...
  start_process("mbmapupdate_img");
  update_mbmap_img(bbxyxy,zcutoff,use_update);
  start_process("");
}
...
bool update_edto_zmap(float collision_radius){
  ...
  bbmin.z = c2x(bbmnmnx_rc[4]);
  bbmax.z = r2y(bbmnmnx_rc[5]);
  //ROS_INFO("UPDAT_EDTO");
  octree.get()->getMetricMin(bbmin_octree.x,bbmin_octree.y,bbmin_octree.z);
  octree.get()->getMetricMax(bbmax_octree.x,bbmax_octree.y,bbmax_octree.z);
  ...
  if(fmax(bbmax.z,bbmin.z) > bbmax_octree.z)
    bbmin.z = fmin(bbmax_octree.z - 5,bbmin.z);
    ...
    for(int y = ymin; y < ymax; y++){
      for(int x = xmin; x < xmax; x++){
        ...
        if(img_height.at<cv::Vec3b>(y2r(y),x2c(x))[2] < z && edf_ptr.get()->getDistance(p) < collision_radius){
          img_height.at<cv::Vec3b>(y2r(y),x2c(x))[2] = z;
        }
      }
    }
  }
...
void merge_update_with_grid(){
  ...
  bbvec[3] = 0;  bbvec[4] = 100;  bbvec[5] = 0;
...
void test_updatembm_cb(const geometry_msgs::PointStamped::ConstPtr& msg){
  if(msg->header.frame_id == ""){
    test_updatembma(perfalt_pnt.point,50,msg->point.z,false);
  }
  else if(msg->header.frame_id == "t"){
      geometry_msgs::Point p0;
      test_updatembma(p0,300,msg->point.z,false);
  }
  else
    test_updatembma(perfalt_pnt.point,50,msg->point.z,true);
}
...
//img_height = cv::imread(par_workdir,CV_LOAD_IMAGE_COLOR);
  ...
  ros::Subscriber sad = nh.subscribe("/tb_testing",10,test_updatembm_cb);
  ...
  bool update_mbmap_needed = false;
  while(ros::ok()){
    update_pos_vlp();
