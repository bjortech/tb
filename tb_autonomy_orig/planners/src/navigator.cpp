...
void draw_delta_height(){
  cv::Mat img_1(1000,1000,CV_8UC3,cv::Scalar(0, 0, 0)); //create image, set encoding and size, init pixels to default val
  cv::Mat img_2(1000,1000,CV_8UC3,cv::Scalar(0, 0, 0)); //create image, set encoding and size, init pixels to default val
  cv::Mat img_d(1000,1000,CV_8UC3,cv::Scalar(0, 0, 0)); //create image, set encoding and size, init pixels to default val
  for(int c = 0; c < img_height.cols; c++){
    for(int r = 0; r < img_height.rows; r++){
      img_1.at<cv::Vec3b>(r,c)[1] = img.at<cv::Vec3b>(r,c)[1];
      img_2.at<cv::Vec3b>(r,c)[2] = img.at<cv::Vec3b>(r,c)[2];
      if(img_1.at<cv::Vec3b>(r,c)[1] > 0 && img_2.at<cv::Vec3b>(r,c)[2] > 0){
        if(img_1.at<cv::Vec3b>(r,c)[1] > img_2.at<cv::Vec3b>(r,c)[2]){
      //    ROS_INFO("1 > 2: %i > %i (%i)",img_1.at<cv::Vec3b>(r,c)[1],img_2.at<cv::Vec3b>(r,c)[2],(img_height.at<cv::Vec3b>(r,c)[1] - img_height.at<cv::Vec3b>(r,c)[2]));
           img_d.at<cv::Vec3b>(r,c)[1] = (img_height.at<cv::Vec3b>(r,c)[1] - img_height.at<cv::Vec3b>(r,c)[2])*15;
        }
        else{
        //  ROS_INFO("1 < 2: %i > %i (%i)",img_1.at<cv::Vec3b>(r,c)[1],img_2.at<cv::Vec3b>(r,c)[2], (img_height.at<cv::Vec3b>(r,c)[2] - img_height.at<cv::Vec3b>(r,c)[1]));
          img_d.at<cv::Vec3b>(r,c)[2] = (img_height.at<cv::Vec3b>(r,c)[2] - img_height.at<cv::Vec3b>(r,c)[1])*15;
        }
      }
      img.at<cv::Vec3b>(r,c)[2] = img.at<cv::Vec3b>(r,c)[2]*5;
      img.at<cv::Vec3b>(r,c)[1] = img.at<cv::Vec3b>(r,c)[1]*5;
    }
  }
  cv::imwrite("/home/nuc/brain/fullpath/"+std::to_string(count_target_paths)+"navmg_1.png",img_1);
  cv::imwrite("/home/nuc/brain/fullpath/"+std::to_string(count_target_paths)+"navmg_2.png",img_2);
  cv::imwrite("/home/nuc/brain/fullpath/"+std::to_string(count_target_paths)+"navmg_d.png",img_d);
}
void drawimg(){
...
void update_mbmap_img(std::vector<int> bbxyxy,float zcutoff){
  int xmin = bbxyxy[0];
  int ymin = bbxyxy[1];
  int xmax = bbxyxy[2];
  int ymax = bbxyxy[3];
...
void test_updatembma(geometry_msgs::Point midpoint,float radlen_xy,float zcutoff){
  std::vector<int> bbxyxy;
  bbxyxy.resize(4);
  bbxyxy[0] = int(midpoint.x-radlen_xy);
  bbxyxy[1] = int(midpoint.y-radlen_xy);
  bbxyxy[2] = int(midpoint.x+radlen_xy);
  bbxyxy[3] = int(midpoint.y+radlen_xy);
  if(got_map)
    update_mbmap_img(bbxyxy,zcutoff);
}
int request_floodfill_slope(geometry_msgs::Point seed_point,int roi_rad){
  int seed_c = x2c(seed_point.x);
  int seed_r = y2r(seed_point.y);
 	int dst0 = img_height_m.at<uchar>(seed_r,seed_c);
  ...
  cv::Rect ccomp;
  img_height_m.copyTo(slopeimage_mono_copy);
  geometry_msgs::Point pTopLeft,pBottomRight;
  pBottomRight.x = seed_point.x + roi_rad;
  pBottomRight.y = seed_point.y - roi_rad;
  pTopLeft.x = seed_point.x - roi_rad;
  pTopLeft.y = seed_point.y + roi_rad;

  //cv::Rect rRect(pTopLeft, pBottomRight);
//  cv::Rect rRect(pymax_xmin, pymin_xmax);
//	cv::Rect rect_roi(seed_c-roi_rad,seed_r-roi_rad,roi_rad*2,roi_rad*2);
	cv::Rect rect_roi(pnt2cv(pTopLeft),pnt2cv(pBottomRight));
  int lo = 3;
  int hi = 3;
  ROS_INFO("Dst0: %i,lo: %i hi: %i,topleft: %.0f %.0f botright: %.0f %.0f",dst0,lo,hi,pTopLeft.x,pTopLeft.y,pBottomRight.x,pBottomRight.y);
  //cv::imwrite("/home/nuc/brain/process/"+std::to_string(count_target_paths)+"height_mono_copy2.png", slopeimage_mono_copy);
  for(int i = 0; i < 10; i++){
    img_height_m.copyTo(slopeimage_mono_copy);
    int area  = cv::floodFill(slopeimage_mono_copy,seed_pnt,cv::Scalar(50), &rect_roi, cv::Scalar(i),cv::Scalar(i), flags);
    ROS_INFO("FLOODFILL_MONO[x %.2f y %.2f] ABS area: %i",seed_point.x,seed_point.y,area);
    if(area < 100000){
      cv::rectangle(slopeimage_mono_copy, pnt2cv(pTopLeft),pnt2cv(pBottomRight),cv::Scalar(255),1,8,0);
      cv::circle(slopeimage_mono_copy,pnt2cv(seed_point),2,cv::Scalar(255),1);
      putText(slopeimage_mono_copy,"z:"+std::to_string(dst0), pnt2cv(seed_point),
          cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(255), 1, CV_AA);
      cv::imwrite("/home/nuc/brain/process/"+std::to_string(count_target_paths)+std::to_string(i)+"height_mono_copy.png", slopeimage_mono_copy);
    }
  }
//	cv::imwrite("/home/nuc/brain/process/"+std::to_string(count_target_paths)+"height_mono_copy.png", slopeimage_mono_copy);
  return 1;
}
////////////////////////////**********************************/////////////////////////////
////////////////////////////**********************************/////////////////////////////
...
void merge_update_with_grid(){
  ...
  for(int c = 0; c < img_height.cols; c++){
    for(int r = 0; r < img_height.rows; r++){
      if(img_update.at<cv::Vec3b>(r,c)[2] > 0){
        if(r < bbvec[0])
          bbvec[0] = r;
        if(c < bbvec[1])
          bbvec[1] = c;
        ...
        if(img_update.at<cv::Vec3b>(r,c)[2] > 0){
          if(first){
            first =false;
            bbvec[4] = img_update.at<cv::Vec3b>(r,c)[2]-1;
            bbvec[5] = img_update.at<cv::Vec3b>(r,c)[2]+1;
          }
          else{
            if(img_update.at<cv::Vec3b>(r,c)[2] > bbvec[5])
              bbvec[5] = img_update.at<cv::Vec3b>(r,c)[2];
            if(img_update.at<cv::Vec3b>(r,c)[2] < bbvec[4])
              bbvec[4] = img_update.at<cv::Vec3b>(r,c)[2];
...
void update_state_forwpath2(){
  if(path_forwsim.poses.size() > 2){
    ...
    for(int i = 0; i < path_forwsim.poses.size(); i++){
      std::string h = path_forwsim.poses[i].header.frame_id;
      float dst = get_dst2d(path_forwsim.poses[i].pose.position,pos);
      float dt = (path_forwsim.poses[i].header.stamp - path_forwsim.poses[0].header.stamp).toSec();
      ROS_INFO("FORWSIM[%i / %i]: %.2f dst %.2f dt z: %.2f state: %h",i,path_forwsim.poses.size(),dst,dt,path_forwsim.poses[i].pose.position.z,h);
      if(h == "got_edto")
        cnt_edto++;
      if(h == "no_data")
        cnt_nodata++;
...
//  ROS_INFO("Point %.0f %.0f - zmax: %.0f a_grid: %i",x,y,z_grid,a_grid);
  if(z_grid > 0 || a_grid > 0){
    ...
    if(z_edto > 0){
      ROS_INFO("grid %.2f edto %.2f mi %i",z_grid,z_edto,get_zmi(pnt.point));
...
void mbtest_cb(const std_msgs::UInt8::ConstPtr& msg){
  test_updatembma(perfalt_pnt.point,100,msg->data);
}
...
//img_height = cv::imread(par_workdir,CV_LOAD_IMAGE_COLOR);
  ...
  ros::Subscriber sta   = nh.subscribe("/test_mb",1,mbtest_cb);
  ...
  //  clean_forwpath2();
    ...
    //int area = request_floodfill_slope(perfalt_pnt.point,50);
    ...
    if(update_mbmap_needed)
      test_updatembma(perfalt_pnt.point,par_maprad*3,perfalt_pnt.point.z+2);


  //  float inclination_target = get_inclination_target();
  //  tilt_error.data = inclination_target;
    ROS_INFO("STATES: [mb: %s fw[%i]: %s ] [gp: %s (%.0f) / pa: %s (%.0f) / pos: %s (%.0f)] sp: %s",state_mb.c_str(),path_forwsim.poses.size(),state_forwpah.c_str(),state_gp_pnt.c_str(),gp_pnt.point.z,state_perfalt.c_str(),perfalt_pnt.point.z,state_pos_pnt.c_str(),pos_pnt.point.z,state_scanpoint.c_str());
    drawimg();
