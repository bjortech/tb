...
geometry_msgs::PolygonStamped poly_down,poly_roi2d,poly_roi;

ros::Publisher pub_get_down,pub_path_to_cluster;
nav_msgs::Path path_getting_processed,path_down,path_starsquare,path_down_full;
...
{
  ros::init(argc, argv, "tb_abmap_pathclustering_down_node");
  ros::NodeHandle nh;
  ros::NodeHandle private_nh("~");
	...
	ros::Subscriber os1 = nh.subscribe("/tb_down_manual/pnt",1,manual_roi_cb);
	ros::Subscriber os2 = nh.subscribe("/tb_down_manual/par",1,manual_par_cb);
	pub_path_to_cluster = nh.advertise<nav_msgs::Path>("/tb_process/get_path_down_clusters",10);

	ros::Subscriber os3 = nh.subscribe("/tb_process/path_down_clusters",1,path_clusters_cb);
	ros::Subscriber os4 = nh.subscribe("/tb_process/paths_down_bbpolys",1,path_bbpolys_cb);
	ros::Subscriber os = nh.subscribe("/tb_edto/down",1,path_down_cb);
	pub_get_down	  	 = nh.advertise<geometry_msgs::PolygonStamped>("/tb_edto/get_down",10);

	ros::Publisher pub_path_master = nh.advertise<nav_msgs::Path>("/tb_vis/down_path_master",10);
	ros::Publisher pub_poly_master = nh.advertise<geometry_msgs::PolygonStamped>("/tb_vis/down_poly_master",10);
	ros::Publisher pub_path_master_segments = nh.advertise<nav_msgs::Path>("/tb_vis/down_pathsegments",10);
	ros::Publisher pub_poly_master_segments = nh.advertise<geometry_msgs::PolygonStamped>("/tb_vis/down_polysegments",10);
  ...
  while(ros::ok()){
    ...
    ros::spinOnce();
		...
		if(down_size > 500 && !path_sent){
			send_path_to_processing(path_down);
		}
		else if(path_sent){
			if(path_clusters_received && path_bbpolys_received){
				...
				count_target_paths++;
				cv::imwrite("/home/nuc/brain/clusters/process_down"+std::to_string(count_target_paths)+".png",img);
				start_process("");
				get_next_request(use_random);
			}
		}
