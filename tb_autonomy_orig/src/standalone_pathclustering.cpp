...
void split_paths_and_find_polys(nav_msgs::Path pathin){
	...
	for(int i = 0; i < path_clusters.size(); i++){
		poly_clusters.push_back(get_bounding_polygon_drawdebug(path_clusters[i],32,extra_length,false));
...
void process_clusters(nav_msgs::Path pathin){
	...
	for(int i = 0; i < path_clusters.size(); i++){
		poly_clusters[i] = get_bounding_polygon_drawdebug(path_clusters[i],32,extra_length,false);
