...
geometry_msgs::PolygonStamped get_bounding_polygon(nav_msgs::Path pathin, int num_points, float extra_length,bool new_img){
	if(new_img)
...
void split_paths_and_find_polys(nav_msgs::Path pathin){
	std::vector<nav_msgs::Path> path_clusters = get_clusters(path_side,4.0);
	std::vector<geometry_msgs::PolygonStamped> poly_clusters;
	for(int i = 0; i < path_clusters.size(); i++){
		poly_clusters.push_back(get_bounding_polygon(path_clusters[i],32,extra_length,false));
...
void process_clusters(nav_msgs::Path pathin){
	...
	for(int i = 0; i < path_clusters.size(); i++){
		poly_clusters[i] = get_bounding_polygon(path_clusters[i],32,extra_length,false);
...
int main(int argc, char** argv)
{
  ...
  ros::Time last_radialupdate = ros::Time::now();
	...
	ROS_INFO("Starsquare");
	geometry_msgs::PolygonStamped path_starsquare_polybb = get_bounding_polygon(path_starsquare,32,extra_length,true);
  ...
  while(ros::ok()){
	...
	//		geometry_msgs::PolygonStamped polyout;
	//		polyout = get_bounding_polygon(path_side,32,extra_length,true);
	//		pub_scanpoint.publish(polyout);
	//	}
		if(side_size > 0 && path_side.poses.size() > 0)
			split_paths_and_find_polys(path_side);
		else
			get_next_request();
		pub_starpoly.publish(path_starsquare_polybb);
	}
	return 0;
}
//
