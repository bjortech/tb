...
bool is_point_visited(nav_msgs::Path path_to_check,geometry_msgs::Point pin,float lim){
  float res,dst;
...
bool is_point32_visited(nav_msgs::Path path_to_check,geometry_msgs::Point32 pin,float lim){
  float res,dst;
...
nav_msgs::Path remove_path(nav_msgs::Path path_to_check,nav_msgs::Path pathin,float cutoff_dst){
	...
	pathout.header = hdr();
	for(int i = 0; i < pathin.poses.size(); i++){
		if(!is_point_visited(path_to_check,pathin.poses[i].pose.position,cutoff_dst)){
			pathout.poses.push_back(pathin.poses[i]);
		}
	}
	return pathout;
}
geometry_msgs::PolygonStamped remove_poly_visited(nav_msgs::Path path_to_check,geometry_msgs::PolygonStamped polyin, float visited_dst_cutoff){
	geometry_msgs::PolygonStamped polyout;
	polyout.header = hdr();
	for(int i = 0; i < polyin.polygon.points.size(); i++){
		if(is_point32_visited(path_to_check,polyin.polygon.points[i],visited_dst_cutoff)){
...
void poly_obstacles_cb(const geometry_msgs::PolygonStamped::ConstPtr& msg){
	...
	poly_obstacles = remove_poly_visited(path_visited,*msg,cluster_visited_cutoff);
	if(poly_obstacles.polygon.points.size() > 0)
...
int main(int argc, char** argv)
{
  ros::init(argc, argv, "tb_targertes_node");
	...
	ros::Subscriber s1 = nh.subscribe("/tb_edto/poly_obstacles",10,poly_obstacles_cb);
