...
geometry_msgs::Point last_pos,bbmin_custom,bbmax_custom,bbmin_octree,bbmax_octree,pos;
...
sensor_msgs::LaserScan rayranges,rayranges2;
float mapradtouse;
...
geometry_msgs::PointStamped pos0,posM,posL,posR,pos0_obs,posM_obs,posL_obs,posR_obs;
ros::Time last_sec;
...
bool update_edto_vlp(float collision_radius,float z0,float z1){
  ...
  ROS_INFO("FAILED update_edto FAILED: zmax_touse: %.2f, zmin_touse: %.2f, collision_radius %.2f, z0 %.2f, z1 %.2f, bbmin_octree: %.2f, : %.2f, : %.2f, bbmax_octree: %.2f, : %.2f, : %.2f ",zmax_touse
  ,zmin_touse,collision_radius,z0,z1,bbmin_octree.x,bbmin_octree.y,bbmin_octree.z,bbmax_octree.x,bbmax_octree.y,bbmax_octree.z);
...
//  update_edto(pnt_midpoint.point,collision_radius,mapradtouse+collision_radius,bbmin_vlp.z-2,bbmax_vlp.z,false);
  ...
  //  ROS_INFO("point[%i] ended after %.0f meters - out_of_bounds: %i going_strong: %i",i,cur_ray_len,out_of_bounds,going_strong);
    ...
    if(going_strong){
      ...
      octomap::point3d closestObst;
      point3d p(ps_clear.pose.position.x,ps_clear.pose.position.y,ps_clear.pose.position.z);
      float yaw, pitch;
      edf_ptr.get()->getDistanceAndClosestObstacle(p,distance,closestObst);
      path_vlp_connect.poses.push_back(ps);
      ps_clear.header.frame_id  = "closed";
      yaw   = atan2(closestObst.y() - cur_vec.y(),closestObst.x() - cur_vec.x());
      pitch = atan2(cur_vec.z() - closestObst.z(),cur_ray_len);
      ps_clear.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,pitch,yaw);
      path_clear.poses.push_back(ps_clear);
      ps_closest.pose.position.x  = closestObst.x();
      ps_closest.pose.position.y  = closestObst.y();
      ps_closest.pose.position.z  = closestObst.z();
      ps_closest.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,pitch,yaw);
      path_vlp_full.poses.push_back(ps_closest);
    }
    else{
...
void checktf(){
  ...
  try{
	...
	if((get_dst3d(pos,last_pos) > 5.0) || (abs(get_shortest(pos_yaw,last_yaw)) > 1.0) || ((ros::Time::now()-last_sec).toSec() > 5.0)){
		last_pos = pos;
		last_yaw = pos_yaw;
...
tb_msgsrv::PathsStamped merge_pathcombs(tb_msgsrv::PathsStamped paths1,tb_msgsrv::PathsStamped paths2){
...
void check_points(float dst,float da_pitch,float da_yaw){
  geometry_msgs::PointStamped p0,pr,pl,pm;
  ros::Time t0 = ros::Time::now();
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher pub_cleared_connect = nh.advertise<nav_msgs::Path>("/tb_path/cleared_connect",100);
  ...
  ros::Publisher pub_superpath_down = nh.advertise<tb_msgsrv::PathsStamped>("/tb_path/superpath_down",100);
  ros::Publisher pub_superpath_side = nh.advertise<tb_msgsrv::PathsStamped>("/tb_path/superpath_side",100);
  ros::Publisher pub_superpath      = nh.advertise<tb_msgsrv::PathsStamped>("/tb_path/superpath",10);
  ...
  while(ros::ok()){
    ...
    if(got_map){
      ...
      pub_cleared_connect.publish(path_vlp_connect);
    ...
    //  tb_msgsrv::PathsStamped paths_down = ;
    //  tb_msgsrv::PathsStamped paths_side = ;
      ...
      //  superpath      = merge_superpaths(superpath_down,superpath_side);
        ...
        tb_msgsrv::PathsStamped paths_down = get_path_clusters(path_raw_down,par_d_r,par_d_zlo,par_d_zhi);
        tb_msgsrv::PathsStamped paths_side = get_path_clusters(path_raw_side,par_s_r,par_s_zlo,par_s_zhi);
        //ROS_INFO("paths_side");
