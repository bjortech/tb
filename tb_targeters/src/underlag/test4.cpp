...
void update_targets(){
	...
	if(path_building_active.poses.size() > 0){
		int ci = getinpath_closestindex2d(path_building_active,pos);
	...
	//	path_organize_1 = get_active_cluster(path_building_active,path_building_active.poses[ci].pose.position);
		nav_msgs::Path path_building_active_cluster = get_active_cluster(path_building_active,path_building_active.poses[ci].pose.position);
		nav_msgs::Path path_active_notvstd 					= get_new_path(path_visited,path_building_active_cluster,5.0,true);
	...
	//		ROS_INFO("A sent: %.2f",a_sent);
		...
		ROS_INFO("UPDATE BUILDING TARGETS, building paths: %i above %i below %i",path_building_active.poses.size(),path_building_active_cluster.poses.size(),path_active_notvstd.poses.size());
		float hdng_end 		= get_hdng(path_active_notvstd.poses[path_active_notvstd.poses.size()-1].pose.position,building_centroid.point);
		float hdng_start 	= get_hdng(path_active_notvstd.poses[0].pose.position,building_centroid.point);
		current_rad = get_hdng(pos,building_centroid.point);
		path_active_rightway.header = hdr();
			ROS_INFO("UPDATE BUILDING TARGETS, min_rad: %.2f -> %.2f current: %.2f",hdng_start,hdng_end,current_rad);
