...
nav_msgs::Path path_building_active,path_building_targets,path_spiral,path_targets,path_targets_sent;
...
void draw_polys(std::string info,std::vector<geometry_msgs::PolygonStamped> polysin,int color_strength){
	...
	for(int i = 0; i < polysin.size(); i++){
		if(polysin[i].polygon.points.size() > 2){
			...
			std::string info = info + std::to_string(i) + ": " + std::to_string(polysin[i].polygon.points.size()) + write_xyz(p_cen);
			count_color++;
			if(count_color == 3)
				count_color = 0;
			if(polysin[i].polygon.points.size() > 3)
...
void get_obstacles_ffill(std::string type){
	...
	int maprad_used = 50;
	polys_heightimage_clustered = get_obstacles(type,pos,maprad_used,5,50);
	int biggest = 0;
	int biggest_i = 0;
	int pnts_count = 0;
	...
	cv::imwrite("/home/nuc/brain/clusters/"+std::to_string(cnt)+"get_obstacles_ffill.png",img_ff_bld);
...
void side2d_path_building_cb(const nav_msgs::Path::ConstPtr& msg){
	ROS_INFO("side2d_path_building_cb");
	check_path_proc(*msg);
}
...
int main(int argc, char** argv)
{
  ros::init(argc, argv, "tb_targertes_node");
	...
	ros::Subscriber s6 = nh.subscribe("/tb_edto/side2d_path_building",10,side2d_path_building_cb);
	...
	while(ros::ok()){
		...
		if((ros::Time::now() - t_start).toSec() > 1.0){
			ccnt++;
