...
bool got_map = false;
ros::Publisher pub_path,pub_target;
...
std::string buildingstate = "idle";
ros::Time buildingstate_change;
geometry_msgs::PointStamped building_centroid;
void set_buildingstate(std::string newstate){
  if(newstate != buildingstate){
    float dt = (ros::Time::now() - buildingstate_change).toSec();
    ROS_INFO("buildingstate: buildingstate: %s -> %s (%.3f seconds in state)",buildingstate.c_str(),newstate.c_str(),dt);
...
float y2r(float y){
  return (img_mono.rows / 2 - y);
}
float x2c(float x){
  return (x + img_mono.cols/2);
}
int r2y(float r){
  return int((img_mono.rows / 2 - r));
}
int c2x(float c){
  return int((c - img_mono.cols / 2));
}
...
bool is_point32_visited(geometry_msgs::Point32 pin,float lim){
  float res,dst;
...
geometry_msgs::PolygonStamped remove_visited(geometry_msgs::PolygonStamped polyin){
	...
	for(int i = 0; i < polyin.polygon.points.size(); i++){
		if(!is_point32_visited(polyin.polygon.points[i],7)){
...
geometry_msgs::Point get_next_endpoint(geometry_msgs::Point endpoint){
	...
	cv::Rect ccomp;
	int area = cv::floodFill(img_mono,cv::Point(c,r),cv::Scalar(150), &ccomp, cv::Scalar(1),cv::Scalar(1), flags);
	for(int x = oct_xmin; x < oct_xmax; x++){
		for(int y = oct_xmin; y < oct_xmax; y++){
			if(img_mono.at<uchar>(y2r(y),x2c(x)) == 150){
				geometry_msgs::Point32 p;
				p.x = x; p.y = y; p.z = endpoint.z;
				if(is_point32_visited(p,7) && get_dst2d3(p,endpoint)){
					best_p = p;
				}
			}
		}
	}
	cv::circle(img_mono,pnt322cv(best_p),1,cv::Scalar(0,0,200),1);
	cv::line (img_mono, pnt2cv(endpoint), pnt322cv(best_p), cv::Scalar(0,200,0),1,cv::LINE_8,0);
...
void get_side(){
	img_blank_mono.copyTo(img_mono);
	    ...
	    for(int x = oct_xmin; x < oct_xmax; x++){
	      ...
	      if(abs(edf_ptr.get()->getDistance(p) - par_dst_target) < par_dst_margin)
					img_mono.at<uchar>(y2r(y),x2c(x)) = 100;
		}
  }
	cv::imwrite("/home/nuc/brain/cluster/"+std::to_string(cnt)+"clusters.png",img_mono);
}
...
std::vector<float> get_bbvec_from_endpoint(geometry_msgs::Point endpoint){
	...
	bbvec.resize(6);
	bbvec[0] = endpoint.x - (par_maprad + par_dst_target);
	bbvec[1] = endpoint.y - (par_maprad + par_dst_target);
	bbvec[2] = endpoint.z - par_dz;
	bbvec[3] = endpoint.x + (par_maprad + par_dst_target);
	bbvec[4] = endpoint.y + (par_maprad + par_dst_target);
...
void set_target(geometry_msgs::Point target_point,float target_point_yaw){
	set_buildingstate("approaching_building");
	building_centroid.point.x = target_point.x + par_dst_target * cos(target_point_yaw);
	building_centroid.point.y = target_point.y + par_dst_target * sin(target_point_yaw);
	building_centroid.point.z = target_point.z;
	last_endpoint = target_point;
	geometry_msgs::PoseStamped ps;
	ps.pose.position = last_endpoint;
	ps.header = hdr();
...
void targetpath_side_cb(const nav_msgs::Path::ConstPtr& msg){
	nav_msgs::Path targetpath_in = remove_visited(*msg,8);
	if(buildingstate == "idle" && targetpath_in.poses.size() > 0 && targetpath_in.poses[0].pose.position.z > 8){
		int ci = getinpath_closestindex2d(targetpath_in,last_endpoint);
		set_target(last_endpoint,tf::getYaw(targetpath_in.poses[ci].pose.orientation));
	}
}
...
void update_state(){
	if(buildingstate == "approaching_building")
		set_buildingstate("inspecting_building");
}
...
int main(int argc, char** argv)
{
  ros::init(argc, argv, "tb_edto_side_auto_node");
	...
	ros::Subscriber s3  = nh.subscribe("/tb_test/targetpath_side",1,targetpath_side_cb);
	...
	std::string state = "approaching_building";
	while(ros::ok()){
			...
			if(last_endpoint.z > 0){
				...
				if(!outside_octomap(bbvec)){
					if(update_edto_bbvec(bbvec,par_dst_target+par_dst_margin)){
						...
						cnt++;
						cv::imwrite("/home/nuc/brain/cluster/"+std::to_string(cnt)+"clusters_down.png",img_mono);
					}
				}
			}
			else{
