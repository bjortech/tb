...
geometry_msgs::PolygonStamped building_obstacle_points,building_polygon,building_polygon_min,poly_cleared,poly_obstacles;
...
std::vector<float> get_poly_bbvec(geometry_msgs::PolygonStamped polyin){
...
void drawimg(){
	...
	int count_color = 0;
	for(int i = 0; i < poly_clusters.size(); i++){
		geometry_msgs::Point p_cen = get_ave_pnt_poly(poly_clusters[i]);
		std::string info = "c#" + std::to_string(i) + ": " + std::to_string(poly_clusters[i].polygon.points.size()) + write_xyz(p_cen);
		count_color++;
		if(count_color == 3)
			count_color = 0;
		cv::Scalar color = get_shifting_color(count_color,100);
		if(poly_clusters[i].polygon.points.size() > 3)
			draw_poly(info,poly_clusters[i],0,0,0,true,color,true);
		else
			draw_poly(info,poly_clusters[i],0,0,0,true,color,true);
	}
	...
	draw_path("targets",path_targets,2,2,4,false,cv::Scalar(200,100,20),true);
	...
	if(building_polygon_min.polygon.points.size() > 0)
		draw_poly("building_min",building_polygon_min,0,0,0,false,cv::Scalar(100,200,100),true);
	draw_and_reset_img();
}
...
geometry_msgs::PolygonStamped get_building_ffill_pnt(std::string type,geometry_msgs::Point pnt, int search_radius){
	int c = x2c(pnt.x);
	int r = y2r(pnt.y);
	geometry_msgs::PolygonStamped poly = get_obstacles(pnt,search_radius);
	for(int r = r-search_radius; r < r+search_radius; r++){
		for(int c = c-search_radius; c < c+search_radius; c++){
		//	if(r > rmn && r < rmx && c > cmn && c < cmx){
				if(type == "side")
					img_ff_bld.at<uchar>(r,c) = img_height.at<cv::Vec3b>(r,c)[1];
				else if(type == "down")
					img_ff_bld.at<uchar>(r,c) = img_height.at<cv::Vec3b>(r,c)[2];
				else
					img_ff_bld.at<uchar>(r,c) = fmax(img_height.at<cv::Vec3b>(r,c)[1],img_height.at<cv::Vec3b>(r,c)[2]);
				if(img_ff_bld.at<uchar>(r,c) == 0)
					img_ff_bld.at<uchar>(r,c) = 255;
		}
	}
	cv::imwrite("/home/nuc/brain/clusters/"+std::to_string(cnt)+"img_ff_bld.png",img_ff_bld);
}*/

int floodfill_actual(geometry_msgs::Point32 p0,int fillcolor){
	int ffillMode = 1;  int connectivity = 8;  int newMaskVal = 255;
	int flags = connectivity + (newMaskVal << 8) +
			 (ffillMode == 1 ? cv::FLOODFILL_FIXED_RANGE : 0);
	cv::Rect ccomp;
	cnt2++;
	int z0 = img_height.at<uchar>(y2r(p0.y),x2c(p0.x));
	if(z0 == 255)
		return 0;
	int lo = 1;
	int hi = 30;
	img_ff_bld.copyTo(img_ff_bld_copy);
	int area = cv::floodFill(img_ff_bld,cv::Point(x2c(p0.x),y2r(p0.y)),cv::Scalar(fillcolor), &ccomp, cv::Scalar(lo),cv::Scalar(hi), flags);
	cv::imwrite("/home/nuc/brain/clusters/"+std::to_string(cnt)+"img_ff_bld.png",img_ff_bld);
	img_ff_bld = img_ff_bld_copy;
...
std::vector<geometry_msgs::PolygonStamped> get_obstacles(std::string type,geometry_msgs::Point midpoint, int radlen_xy){
	prep();
	poly_clusters.resize(0);
	geometry_msgs::PolygonStamped polyout;
	polyout.header = hdr();
  int c1 = x2c(midpoint.x-radlen_xy);
  int c0 = x2c(midpoint.x+radlen_xy);
  ...
  for(int c = fmin(c1,c0); c < fmax(c1,c0); c++){
    for(int r = fmin(r1,r0); r < fmax(r1,r0); r++){
			if(type == "side")
				obs_z = img_height.at<cv::Vec3b>(r,c)[1];
			else if(type == "down")
	...
	ROS_INFO("IMG UPDATE COMPLETE");
	geometry_msgs::PolygonStamped poly_cluster;
	int size_last = 5;
	while(polyout.polygon.points.size() > 5 && size_last > 0){
		int fillcolor = 150;
		int area = floodfill_actual(polyout.polygon.points[0],fillcolor);
		geometry_msgs::PolygonStamped poly_remaining_new;
		for(int i = 0; i < polyout.polygon.points.size(); i++){
			int r = y2r(polyout.polygon.points[i].y);
			int c = x2c(polyout.polygon.points[i].x);
			if(img_ff_bld.at<uchar>(r,c) == fillcolor)
				poly_cluster.polygon.points.push_back(polyout.polygon.points[i]);
			else
				poly_remaining_new.polygon.points.push_back(polyout.polygon.points[i]);
		}
		size_last = poly_cluster.polygon.points.size();
		polyout = poly_remaining_new;
		poly_cluster.header.frame_id = "map";
		poly_clusters.push_back(poly_cluster);
...
void get_obstacles_ffill(std::string type){
	...
	if(poly_obstacles.polygon.points.size() < 5){
		return;
	}
	if(state_building == "looking_for_building"){
		poly_obstacles = sort_poly_around_dst(poly_obstacles);
		pnt.x = poly_obstacles.polygon.points[0].x;
		pnt.y = poly_obstacles.polygon.points[0].y;
		pnt.z = poly_obstacles.polygon.points[0].z;
	}
	poly_clusters = get_obstacles(type,pnt,40);
	int biggest = 0;
	int bi = 0;

	for(int i = 0; i < poly_clusters.size();i++){
		if(poly_clusters[i].polygon.points.size() > biggest){
			biggest = poly_clusters[i].polygon.points.size();
			bi = i;
			ROS_INFO("B#i: %i",i,biggest);
		}
	}
//	if(biggest > 0 && poly_clusters[biggest].polygon.points.size() > 5){
//		ROS_INFO("GOTIT");
	//	building_centroid.point = get_ave_pnt_poly(poly_clusters[biggest]);
	//	ROS_INFO("GOTIT2");
//	}
}
...
nav_msgs::Path mergesides(){
	nav_msgs::Path pathout;
	pathout = path_side_norm;
	for(int i = 0; i < path_sideauto_small.poses.size(); i++){
		pathout.poses.push_back(path_sideauto_small.poses[i]);
	}
	for(int i = 0; i < path_sideauto_big.poses.size(); i++){
		pathout.poses.push_back(path_sideauto_big.poses[i]);
	}
	ROS_INFO("Paths merged: norm %i + %i + %i = %i",path_side_norm.poses.size(),path_sideauto_small.poses.size(),path_sideauto_big.poses.size(),pathout.poses.size());
	return pathout;
}

void set_building_polygon(nav_msgs::Path pathin){
	int ci = getinpath_closestindex2d(pathin,pos);
	int fi = getinpath_farthestindex2d(pathin,pos);

	float yaw0 = tf::getYaw(pathin.poses[ci].pose.orientation);
	float yawn = tf::getYaw(pathin.poses[fi].pose.orientation);
	float dhdng = get_shortest(yawn,yaw0);
	float building_direction = get_path_yaw(pathin);

	geometry_msgs::Point32 p;
	building_polygon_min.header = hdr();
	building_polygon_min.polygon.points.resize(0);
	p.x = pathin.poses[ci].pose.position.x;
	p.y = pathin.poses[ci].pose.position.y;
	p.z = pathin.poses[ci].pose.position.z;
	building_polygon_min.polygon.points.push_back(p);
	p.x = pathin.poses[ci].pose.position.x + 20 * cos(yaw0);
	p.y = pathin.poses[ci].pose.position.y + 20 * sin(yaw0);
	p.z = pathin.poses[ci].pose.position.z;
	building_polygon_min.polygon.points.push_back(p);
	p.x = pathin.poses[fi].pose.position.x + 20 * cos(yawn);
	p.y = pathin.poses[fi].pose.position.y + 20 * sin(yawn);
	p.z = pathin.poses[fi].pose.position.z;
	building_polygon_min.polygon.points.push_back(p);
	p.x = pathin.poses[fi].pose.position.x;
	p.y = pathin.poses[fi].pose.position.y;
	p.z = pathin.poses[fi].pose.position.z;
	building_polygon_min.polygon.points.push_back(p);
	building_centroid.point = get_ave_pnt_poly(building_polygon_min);
	building_ranges_at_rad.resize(0);
	building_ranges_at_rad.resize(building_polygon.polygon.points.size());
	sort_poly_around_pnt(building_polygon_min,get_ave_pnt_poly(building_polygon_min));
	set_state_building("approaching_building");
	ROS_INFO("path_targets[%i]: %.2f (building_dir: %.2f (yaw0: %.2f / yawn: %.2f))",ci,building_direction,yaw0,yawn);
...
void update_centroid(){
	int p1,p2,p3;
	p3	= get_completion_percent_orig();
	p1	= get_completion_percent(building_v0,path_visited.poses.size(),building_centroid.point);
	ROS_INFO("Completion: %i / %i",p1,p2);
	building_centroid.point = get_ave_pnt_ni(path_visited,building_v0,path_visited.poses.size());
	ROS_INFO("BLD: Centroid Calculated %.2f %.2f %.2",building_centroid.point.x,building_centroid.point.y,building_centroid.point.z);
}
void update_building_hori(nav_msgs::Path pathin){
	...
	ROS_INFO("update_building_hori_done");
}
void next_target(){
...
void get_target_looking(){
	...
	for(int i =0; i < obstacles_polygons.size(); i++){
		if(obstacles_polygons[i].polygon.points.size() > 0){
			geometry_msgs::Point p_ave = get_ave_pnt_poly(obstacles_polygons[i]);
			if(p_ave.z > pbest.z){
				ps.header = hdr();
				pbest = p_ave;
...
void check_path_proc(nav_msgs::Path pathin){
	...
	path_organize_1 = get_standardized_cluster_simple(pathin);
	...
	//evaluate_roscdpotential_targetpath(path_organize_1);

	//path_organize_2 = create_ordered_path(pathin);
//	ROS_INFO("path1: %i path2: %i",path_organize_1.poses.size(),path_organize_2.poses.size());
}
nav_msgs::Path check_path_proc_new(nav_msgs::Path pathin){
	ROS_INFO("check_path_proc");
	path_organize_1 = remove_path_other(remove_path_visited(get_standardized_cluster_simple(pathin),5.0),path_targets,5);
	ROS_INFO("path_organize_1 %i",path_organize_1.poses.size());
	path_organize_2 = remove_path_other(create_ordered_path(get_standardized_cluster_simple(pathin)),path_targets,5);
	ROS_INFO("path_organize_2 %i",path_organize_2.poses.size());
	if(path_organize_1.poses.size() > path_organize_2.poses.size())
		return path_organize_1;
	else
		return path_organize_2;
}

void update_target_building(){
	//building_centroid.point = get_ave_pnt_ni(path_visited,building_v0,path_visited.poses.size());
	//if(path_sideauto_small.poses.size() > 10){
		/*nav_msgs::Path path_targets_temp = check_path_proc(mergesides());//path_sideauto_small;//check_path_proc(mergesides());
		// get_standardized_cluster_simple(mergesides())),5.0);//check_path_proc(path_sideauto_small);
		if(path_targets_temp.poses.size() > 0){
			if(path_targets.poses.size() > path_targets_i){
				nav_msgs::Path path_cont = get_continous_path(path_targets_temp);
				ROS_INFO("patyh_cont: %i",path_cont.poses.size());
				if(path_cont.poses.size() > path_targets.poses.size()){
					set_target_path(path_cont);
				}
			}
			else
				set_target_path(path_targets_temp);
		}*/
...
//	ROS_INFO("path_side_norm: %i",path_side_norm.poses.size());
//	check_overlap(mergesides());
//	ROS_INFO("path_sideauto_small: %i",path_sideauto_small.poses.size());
//	check_overlap(path_sideauto_small);
//	ROS_INFO("path_sideauto_big: %i",path_sideauto_big.poses.size());
//	check_overlap(path_sideauto_big);
//	ROS_INFO("UPDATE: LOOKING_DONE");
}


void update_approach(){
	ROS_INFO("UPDATE: APPROACH");
...
void check_overlap2(nav_msgs::Path pathin){
	if(pathin.poses.size() < 10 || poly_cleared.polygon.points.size() == 0)
		return;
	std::vector<nav_msgs::Path> paths = get_clusters(pathin);
	for(int i = 0; i < paths.size(); i++){
		int v = abs(get_shortest(tf::getYaw(paths[i].poses[paths[i].poses.size()-1].pose.orientation),tf::getYaw(paths[i].poses[0].pose.orientation) )*10);
		ROS_INFO("Paths[%i]: %i val: %i",i,paths[i].poses.size(),v);
		if((paths[i].poses.size() > 20)){ //&& (abs(get_shortest(tf::getYaw(paths[i].poses[paths[i].poses.size()-1].pose.orientation),tf::getYaw(paths[i].poses[0].pose.orientation) )*10) < 5*M_PI)){
			float cluster_spacing_cutoff = 10;
			init_building(paths[i]);
		//	obstacles_polygons = reorganize_polys(find_clusters(poly_obstacles,cluster_spacing_cutoff));
		}
		//	set_building_polygon(paths[i]);//init_building(paths[i]);
	}
}
...
//	poly_obstacles = remove_poly_path(path_visited,*msg,cluster_visited_cutoff);
	for(int i = 0; i < msg->polygon.points.size(); i++){
		if(poly_obstacles.polygon.points.size() > 0){
			if(!is_point32_in_poly(poly_obstacles,msg->polygon.points[i],1.0))
				poly_obstacles.polygon.points.push_back(msg->polygon.points[i]);
		}
		else
			poly_obstacles.polygon.points.push_back(msg->polygon.points[i]);
	}
...
void update_state(){
	...
	if(state_building == "inspecting_building"){
	//	update_target_building();
	//	update_building_hori(mergesides())
		update_centroid();
		pub_setp_xyz.publish(target_xyz);
	}
}
int main(int argc, char** argv)
{
  ros::init(argc, argv, "tb_targertes_node");
	...
	while(ros::ok()){
		...
		check_target();
		update_state();
		poly_obstacles.header = building_centroid.header = hdr();
		...
		pub_obs_poly.publish(poly_obstacles);
